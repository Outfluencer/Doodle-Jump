...
public class GlyphPage
{
    ...
    public void generateGlyphPage(final char[] chars) {
        ...
        maxWidth += 2.0;
        maxHeight += 2.0;
        this.imgSize =  2 * (int)Math.ceil(Math.max(Math.ceil(Math.sqrt(maxWidth * maxWidth * chars.length) / maxWidth), Math.ceil(Math.sqrt(maxHeight * maxHeight * chars.length) / maxHeight)) * Math.max(maxWidth, maxHeight)) + 1;
    ...
    public void setupTexture() {
        this.loadedTexture = new Texture(this.bufferedImage);
    }
    ...
    static class Glyph
    {
        ...
        static void access$0(final Glyph glyph, final int width) {
            glyph.width = width;
        }
        
        static void access$1(final Glyph glyph, final int height) {
            glyph.height = height;
        }
        
        static void access$4(final Glyph glyph, final int x) {
            glyph.x = x;
        }
        
        static void access$5(final Glyph glyph, final int y) {
            glyph.y = y;
        }
    }
}
